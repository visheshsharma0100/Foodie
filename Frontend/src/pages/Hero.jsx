...
function Hero() {
  return (
    /* pt-20 sm:pt-24 Navbar ke niche proper spacing and safety gap deta hai */
    <section className="pt-20 sm:pt-24 md:pt-28 pb-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 transition-all">
      
      {/* HERO BANNER CONTAINER */}
      <div className="relative w-full min-h-[280px] sm:min-h-[360px] md:min-h-[420px] lg:min-h-[460px] overflow-hidden rounded-2xl sm:rounded-3xl shadow-2xl bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 group flex items-center">
        
        {/* Background Image: High-quality Unsplash food image or masked background to hide text duplicate */}
        <img
          className="absolute inset-0 w-full h-full object-cover object-right sm:object-center group-hover:scale-[1.02] transition-transform duration-700 ease-out opacity-40 sm:opacity-60 mix-blend-luminosity"
          src={HeroImage}
        ...
        {/* Solid & Gradient Mask to completely block/hide any baked-in background text */}
        <div className="absolute inset-0 bg-gradient-to-r from-slate-950 via-slate-900/90 to-transparent z-0 pointer-events-none" />
        <div className="absolute inset-0 bg-slate-950/40 z-0 pointer-events-none" />

        {/* Hero Content Overlay */}
        <div className="relative z-10 p-6 sm:p-10 md:p-12 lg:p-16 max-w-xl sm:max-w-2xl">
          
          {/* Badge */}
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs sm:text-sm font-semibold bg-orange-500/20 text-orange-400 border border-orange-500/30 backdrop-blur-md mb-3 sm:mb-4">
            <span>⚡</span> Fast Delivery & Fresh Food
          </span>

          {/* Clean HTML/CSS Heading */}
          ...
          {/* Subheading */}
          <p className="mt-2 sm:mt-4 text-xs sm:text-base md:text-lg text-slate-300 font-normal line-clamp-2 sm:line-clamp-none max-w-md">
