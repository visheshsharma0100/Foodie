...
/* ==========================================================================
   INTERNAL COMPONENT: STICKY MOBILE CART BAR
   ========================================================================== */
function StickyMobileCartBar() {
  const navigate = useNavigate();
  const { isAuthenticated } = useAuth();
  const { cartItems, getCartTotal } = useCart();

  // Show ONLY when logged in and cart has items
  ...
  return (
    <aside
      ...
      className="fixed bottom-0 left-0 right-0 z-40 md:hidden p-3 pb-[calc(0.75rem+env(safe-area-inset-bottom,0px))] bg-gradient-to-t from-slate-900/40 via-slate-900/10 to-transparent backdrop-blur-[2px] transition-all duration-300 animate-slide-up"
    >
      <div
        ...
        className="w-full bg-slate-900 text-white rounded-2xl p-3.5 px-4 shadow-2xl shadow-slate-900/40 border border-slate-800/80 flex items-center justify-between cursor-pointer active:scale-[0.98] transition-transform duration-150"
      >
        {/* Left Section: Item Count & Total Price */}
        <div className="flex items-center space-x-3">
          <div className="relative flex items-center justify-center w-10 h-10 bg-orange-500/20 text-orange-400 rounded-xl border border-orange-500/30">
            <svg
              className="w-5 h-5"
              fill="none"
              stroke="currentColor"
            ...
            <span className="absolute -top-1.5 -right-1.5 bg-orange-500 text-white text-[10px] font-black w-5 h-5 rounded-full flex items-center justify-center border-2 border-slate-900 shadow-sm animate-pulse">
              {totalQuantity}
            </span>
          </div>
        ...
        <div className="flex items-center space-x-1.5 bg-orange-500 hover:bg-orange-600 text-white font-bold text-sm px-4 py-2.5 rounded-xl shadow-md shadow-orange-500/30 transition-colors">
          <span>View Cart</span>
          <svg
            className="w-4 h-4"
            fill="none"
...
export default function MenuPage() {
  ...
  const showMobileCartBar = isAuthenticated && cartItems && cartItems.length > 0;
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans antialiased selection:bg-orange-500 selection:text-white">
      ...
      {/* MAIN CONTAINER: Dynamic bottom padding to prevent bar from covering cards */}
      <main
        className={`pt-28 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 transition-all duration-300 ${
          showMobileCartBar ? 'pb-32 md:pb-20' : 'pb-20'
        }`}
      >
        {/* TOP SECTION */}
      ...
      {/* Embedded Sticky Mobile Cart Bar */}
      <StickyMobileCartBar />
    </div>
  );
}
